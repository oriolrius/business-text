// SQLite Device Management - Learning Panel Edit
...
function makeInlineEditable(cell) {
  ...
  input.addEventListener('blur', function () {
    ...
    // Update cell content with the input value when losing focus
    const fieldName = this.getAttribute('data-field');
    const newValue = this.value.trim();
    
    // Update the cell's display value
    cell.innerHTML = formatDisplayValue(fieldName, newValue);
    
    // Mark the cell as modified for visual feedback
    cell.style.backgroundColor = theme.colors.warning.transparent + '40';
...
async function saveRow(deviceId, rowIndex) {
  ...
  try {
    ...
    cells.forEach(cell => {
      ...
      // Clear modified styling after successful save
