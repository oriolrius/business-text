...
/**
 * Enhanced Styles Editor with URL fetch functionality
 */
export const StylesWithUrlEditor: React.FC<StandardEditorProps> = (props) => {
  const { onChange, context } = props;
  const [isLoading, setIsLoading] = useState(false);
  
  // Get the current stylesRemoteUrl from panel options
  const options = context.options as PanelOptions;
  const remoteUrl = options.stylesRemoteUrl || '';
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      // Update the styles content with fetched content
      onChange(content);
    } catch (error) {
