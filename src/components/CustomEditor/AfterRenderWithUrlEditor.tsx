...
/**
 * Enhanced After Render Editor with URL fetch functionality
 */
export const AfterRenderWithUrlEditor: React.FC<StandardEditorProps> = (props) => {
  const { onChange, context } = props;
  const [isLoading, setIsLoading] = useState(false);
  
  // Get the current afterRenderRemoteUrl from panel options
  const options = context.options as PanelOptions;
  const remoteUrl = options.afterRenderRemoteUrl || '';
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      // Update the afterRender content with fetched content
      onChange(content);
    } catch (error) {
